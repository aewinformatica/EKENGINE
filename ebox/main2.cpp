...
int main(){
	...
	while ( !key[KEY_ESC] ){
		...
		for ( int q = 0; q < 10; q++  ){
			...
			if ( olist[q].collided ) 
				textprintf_ex(work,font,1,q*10,makecol(255,255,255),-1,"Box %d collided with something!", q );
			else	textprintf_ex(work,font,1,q*10,makecol(255,255,255),-1,"Box %d has not collided");
		}
		textprintf_ex(work,font,1,470,makecol(255,255,255),-1,"Press D to toggle drawing of collision boxes");
